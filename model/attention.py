...
from model.layers import DenseGCNConv,MLP
...
class AttentionLayer(torch.nn.Module):
    ...
    def __init__(self, num_linears, conv_input_dim, attn_dim, conv_output_dim, input_dim, output_dim,
        ...
        for _ in range(input_dim):
            self.attn_dim=attn_dim
            self.attn.append(Attention(conv_input_dim,self.attn_dim,conv_output_dim,num_heads=num_heads,conv=conv))

        self.hidden_m=2*max(input_dim,output_dim)
        self.mlp=MLP(num_linears,2*input_dim,self.hidden_m,output_dim,use_bn=False,activate_func=F.elu)
        self.multi_channel=MLP(2,input_dim*conv_output_dim,self.hidden_m,conv_output_dim,use_bn=False,activate_func=F.elu)
