...
from model.ScoreNetwork_A import ScoreNetworkA
# from sampling import get_pc_sampler
from sde import VPSDE, VESDE
from utils.ema import ExponentialMovingAverage
from model.ScoreNetwork_X import ScoreNetworkX,ScoreNetworkX_GMH
...
def load_sampling_fn(config_train,config_module,config_sample,device):
    ...
    sampling_fn=get_sampler(sde_x=sde_x,sde_adj=sde_adj,shape_x=shape_x,shape_adj=shape_adj,
                            predictor=config_module,corrector=config_module.corrector,
                            snr=config_module.snr,scale_eps=config_module.scale_eps,
                            n_steps=config_module.n_steps,probability_flow=config_sample.probability_flow,
...
def load_ckpt(config, device):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    # path = f'checkpoints/{config.data.data}/{config.ckpt}.pth'
    path = f'checkpoints/{config.data.data}/{config.data.data}.pth'
