...
#     """Construct an EulerMaruyamaPredictor."""
...
#
#
# def get_pc_sample(sde_x,sde_adj,shape_x,shape_adj,predictor="Euler",corrector="None",snr=0.1,scale_eps=1.0
#                   ,n_steps=1,probability_flow=False,continuous=False,denoise=True,eps=1e-3,device="cuda"):
#
...
#         with torch.no_grad():
#             #-------- Initial sample --------
#             x=sde_x.sample_limiting_distribution(shape_x).to(device)
#             adj=sde_adj.marginal_sample_sym(shape_adj).to(device)
#             flags=init_flags
#             x=mask_x(x,flags)
...
#             print(' ')
#             return (x_mean if denoise else x),(adj_mean if denoise else adj),diff_steps*(n_step+1)
#         return pc_sampler
