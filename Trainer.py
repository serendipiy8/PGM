...
import numpy as np
import torch
from utils.loader import load_seed,load_device,load_data,load_model_params,load_model_optimizer,load_ema,load_loss_fn,load_batch
from utils.logger import Logger, train_log, start_log, set_log
import os
...
class Trainer(object):
    ...
    def train(self,ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            """Different Riemannian Product Space."""
            self.train_x_space=[]
            self.train_adj_space=[]
            self.test_x_space=[]
            self.test_adj_space=[]


            """In Each Riemannian Space."""
            self.train_x=[]
            self.train_adj=[]
            self.test_x=[]
            self.test_adj=[]
            ...
            if epoch % self.config.train.save_interval == self.config.train.save_interval - 1:
                ...
                }, f'./checkpoints/{self.config.data.data}/{self.config.data.data}.pth')
