...
from backbone import GAT,GCN,GraphSAGE


EPS = 1e-5
class RiemannianFeature(nn.Module):
    def __init__(self,num_nodes,dimensions,init_curvature,num_factors,learnable=False):
        super(RiemannianFeature,self).__init__()
        ...
        for i in range(num_factors):
            if isinstance(dimensions,list):
                ...
                d=dimensions
            k=init_curvature[i]*(torch.randn(1)+1)
...
class Model(nn.Module):
    ...
    def forward(self, x, edge_index, motif, neg_motif, rm_features: RiemannianFeature):
        # 将输入数据类型转换为 double
        # x = x.flost()
        # edge_index = edge_index.double()
        # motif = motif.double()
        # neg_motif = neg_motif.double()
        ...
        for embed in laplacian:
            ...
            embeds.append(embed)
        loss = cl_loss / len(laplacian)
        embeds = torch.cat(embeds, dim=-1)
        output = self.norm(torch.cat([x, embeds], dim=-1))
        return output, loss
